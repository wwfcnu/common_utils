...
def read_file(file):
    ...
    # Parse each JSON string in the file and append it to the list
    for line in meta_data:
        data_dict = json.loads(line)
        #caption_writing = data_dict["caption_writing"]
        caption_summary = data_dict["caption_summary"]  #为了判断非空
        #pseudo_attribute = data_dict["pseudo_attribute"]
        
        if caption_summary:
            list_of_dicts.append(data_dict)

    # 创建最终的JSON数据
    final_data = {"data": list_of_dicts}
    # data_list = 
    return final_data
...
def translate_file(batch):
    ...
    # 批次翻译caption
    #print(js_caption)
    caption_zh = modelscope_trans(js_caption)
    print(caption_zh)
    ...
    #caption_zh_list = modelscope_trans(caption_list)

    # 更新翻译结果
    for i, js in enumerate(batch):
        js["tags_zh"] = restored_tags_list[i]
        js["caption_summary_zh"] = caption_zh[i]
...
# 按照字符长度将输入列表分成多个批次
    ...
    for item in js_data:
        ...
        caption_writing = item["caption_summary"]
        # print(f"-------------------{id}")
        # print(f"--------------------{description}")
        item_length = len(caption_writing)
...
if __name__ == "__main__":
    ...
    for split in splits:
        file = f"/mnt/data1/AudioDataset/download/modelscope_trans/msd/msd-subsets/msd_{split}.json"
        print(f"正在翻译{split}")
        ...
        with open(f'msd_translate_{split}.json', 'w') as file:

...
#     "__stripe_mid": "03d3e3c7-2767-4343-9fd1-8f730a177380012078",
#     "token": "AOppEoGOOuogZrDoytoCTIPlYywDyrARPsWWyTqVsVnsikGmOqwwniyOInciddOvPFcqXGDKcpLTmsetVnvwqoRblWJISUjalcjHumtNioiiMwjPlWcQjnlCvwaOAsKa",
#     "datadome": "vkwNPACYyGMxF3I5fjdfydyGirSEY00w380Koczia0QU_a2A7a4UbJgcm_SSGCxDrWSNdxQUX4oGdJwgiXlBLJ0HCKl7RWjWqhA5n0KTIB7qIwTQh2gIjHnYtjYHGUkA",
#     "__stripe_sid": "d4d77f95-fa50-4420-ab37-0677b93561f9d9d91e",
#     "aws-waf-token": "c7ba9bcc-2a06-4efe-b858-c0d27082d2a8:AAoAnQhJC/kBAAAA:"
# }
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--sub_path",type=str, help="指定下载的子目录路径，例如: train/zh（不填则下载全部）",default="", required=False)
    # 解析命令行参数
    args = parser.parse_args()
    print(parser)
    sub_path = args.sub_path.strip("/") if args.sub_path else ""
    ...
    # 如果指定了子目录，从子目录开始爬取
    if sub_path:
        url = f"https://hf-mirror.com/{model_id}/tree/main/{sub_path}"
        print(f"指定子目录下载: {sub_path}")
    else:
    ...
    # 如果指定了子目录，元数据文件名加上子目录标识，避免和全量元数据冲突
    if sub_path:
        safe_sub = sub_path.replace("/","_")
        hg_down_info_path = f"{out_dir}/hg_down_info_{safe_sub}.json"
    else:
    ...
    while True:
        ...
        with open(hg_down_info_path,"r") as r:
            ...
            for line in js:
                type = line["type"]
                # 如果指定了子目录，只处理该子目录下的文件和目录
                if sub_path and not line["path"].startswith(sub_path):
                    continue
                if type == "directory":
                    ...
                    # 确保文件所在的父目录存在（子目录下载时父目录可能不在元数据中）
